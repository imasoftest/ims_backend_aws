...
class PresetAppointmentViewSet(viewsets.ModelViewSet):
    ...
    def create(self,request):
        ...
        for i in range(no_of_days+1):
            ...
            mySlotList_each_day = self.get_daily_slots(start=startTime_str, end=endTime_str, slot=slot_time, date=date_required)
...
class PresetAppointmentSessionsViewset(viewsets.ModelViewSet):
    ...
    def downloadSessionDeatils(self, request, *args, **kwargs):
        sessionQuerySet = PresetAppointmentSessions.objects.filter(child_id__isnull=False).values('id','appointment_id','appointment__title','child__first_name','child__last_name','child__nameOfClass','startDate','endDate','startTime','endTime')
